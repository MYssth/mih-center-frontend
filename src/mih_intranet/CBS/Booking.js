...
import CBSSubmtINC from './components/SubmtINC';
import CBSSubmtERR from './components/SubmtERR';
import CBSSubmtComp from './components/SubmtComp';
...
function CBSBooking() {
    ...
    useEffect(() => {
        ...
        fetch(`http://${process.env.REACT_APP_host}:${process.env.REACT_APP_himsPort}/api/hims/getpsndatabyid/${token.personnel_id}`)
        ...
        fetch(`http://${process.env.REACT_APP_host}:${process.env.REACT_APP_cbsPort}/api/cbs/getschedbyreqid/${token.personnel_id}`)
            .then((response) => response.json())
            .then((data) => {
                setSched(data);
            })
    ...
    useEffect(() => {
        const tmpFrDate = `${fromDate?.getFullYear()}-${String(parseInt(fromDate?.getMonth(), 10) + 1).padStart(2, '0')}-${String(fromDate?.getDate()).padStart(2, '0')}T${String(fromTime?.getHours()).padStart(2, '0')}:${String(fromTime?.getMinutes()).padStart(2, '0')}:00.000Z`;
        const tmpToDate = `${toDate?.getFullYear()}-${String(parseInt(toDate?.getMonth(), 10) + 1).padStart(2, '0')}-${String(toDate?.getDate()).padStart(2, '0')}T${String(toTime?.getHours()).padStart(2, '0')}:${String(toTime?.getMinutes()).padStart(2, '0')}:00.000Z`;
        fetchCarData(tmpFrDate, tmpToDate);
    }, [fromDate, fromTime, toDate, toTime]);

    async function fetchCarData(fDate, tDate) {
        fetch(`http://${process.env.REACT_APP_host}:${process.env.REACT_APP_cbsPort}/api/cbs/getfilteredcar/${fDate}/${tDate}/0`)
            .then((response) => response.json())
            .then((data) => {
                setCar(data);
                carFilter(data);
    ...
    function handleSubmit() {
        ...
        if (jsonData.from_date === null || jsonData.to_date === null ||
            jsonData.place === "" || jsonData.province === "" ||
            jsonData.pax_amt === "" || jsonData.detail === "" ||
            jsonData.dept_id === ""
        ) {
            setSubmitINC(true);
            return;
                                        ...
                                        >
                                            ...
                                            <Grid container rowSpacing={{ xs: 2 }} columnSpacing={{ xs: 2 }}>
                                                ...
                                                <Grid item md={3} xs={12}>
                                                    <label className="form-label">เบอร์โทรศัพท์ติดต่อ</label><br />
                                                    <TextField
                                                        fullWidth
                                                ...
                                                <Grid item xs={0} md={5} />
                                                <Grid item md={4}>
                                                    <label className="form-label">วันที่-เวลา</label><br />
                                                ...
                                                <Grid item md={4}>
                                                    <label className="form-label">ถึงวันที่-เวลา</label><br />
                                                    <LocalizationProvider dateAdapter={AdapterDateFns} locale={thLocale}>
                                                        <Stack direction="row" spacing={1}>
                                                            <DatePicker
                                                                disablePast
                                                ...
                                                <Grid item md={2} xs={12}>
                                                    <label className="form-label">จำนวนผู้โดยสาร</label><br />
                                                    <TextField
                                                        fullWidth
                                                        label="จำนวนผู้โดยสาร"
                                                        value={paxAmt}
                                                        onChange={(event) => { setPaxAmt(event.target.value) }}
                                                        sx={{
                                                ...
                                                <Grid item md={4} xs={12}>
                                                    <label className="form-label">สถานที่</label><br />
                                                    <TextField
                                                        fullWidth
                                                        label="สถานที่"
                                                        value={place}
                                                ...
                                                <Grid item md={3} xs={12}>
                                                    <label className="form-label">จังหวัด</label><br />
                                                    <Autocomplete
                                                ...
                                                {duplicate ?
                                                    <Grid item xs={12}>
                                                        <Typography variant='subtitle1' color={'error.main'} > รถคันนี้มีการขอใช้งานซ้ำกับคำขอเลขที่ {duplicate}<br />จาก {dupDept} </Typography>
                                                    </Grid>
                                                    :
                                                    ""}
                                                <Grid item md={12}>
                                                    <Stack direction="row" spacing={1}>
                                                        ...
                                                        <button type="submit" className="btn btn-danger" onClick={() => navigate('/cbsdashboard', { replace: true })}>
                                                            ยกเลิก
                                                        </button>
                                                    </Stack>
                                                </Grid>
