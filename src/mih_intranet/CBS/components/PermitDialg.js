...
function CBSPermitDialg({ openDialg, onCloseDialg, data }) {
    ...
    async function fetchCarData(fDate, tDate, typeId) {
        fetch(`http://${process.env.REACT_APP_host}:${process.env.REACT_APP_cbsPort}/api/cbs/getfilteredcar/${fDate}/${tDate}/${data.id}`)
            .then((response) => response.json())
            .then((result) => {
                setCar(result);
    ...
    const handlePermit = () => {
        ...
        if (jsonData.from_date === null || jsonData.to_date === null ||
            ...
            jsonData.drv_pid === "0" || jsonData.car_type_id === 0 ||
            jsonData.car_id === 0 || jsonData.dept_id === ""
        ) {
            setSubmitINC(true);
            return;
        }
                ...
                <DialogContent>
                    ...
                    <Grid container rowSpacing={{ xs: 2 }} columnSpacing={{ xs: 2 }}>
                        ...
                        {duplicate && duplicate !== id ?
                            <Grid item xs={12}>
                                <Typography variant='subtitle1' color={'error.main'} textAlign={'center'} > รถคันนี้มีการขอใช้งานซ้ำกับคำขอเลขที่ {duplicate}<br />จาก {dupDept} </Typography>
                            </Grid>
                            :
                            ""}
                        <Grid item md={12}>
