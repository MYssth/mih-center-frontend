...
function CBSDenyDialg({ openDialg, onCloseDialg, data }) {
  ...
  const handleDeny = () => {
    ...
    fetch(`${process.env.REACT_APP_host}${process.env.REACT_APP_cbsPort}/denybook`, {
      method: 'POST',
      headers: {
  ...
  return (
    <>
      ...
      <Dialog open={openDialg} onClose={onCloseDialg}>
        <DialogTitle>ยกเลิกคำขอเลขที่ {data?.id}</DialogTitle>
        <DialogContent>
          <Grid container rowSpacing={{ xs: 2 }} columnSpacing={{ xs: 2 }}>
            ...
            <Grid item xs={12}>
              <ValidationTextField required fullWidth id="note" name="note" label="หมายเหตุการยกเลิก" />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
