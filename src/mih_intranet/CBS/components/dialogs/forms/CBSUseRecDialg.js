...
function CBSUseRecDialg({ openDialg, onCloseDialg, data }) {
    ...
    useEffect(() => {
        const tmpFrDate = `${depDate?.getFullYear()}-${String(parseInt(depDate?.getMonth(), 10) + 1).padStart(2, '0')}-${String(depDate?.getDate()).padStart(2, '0')}T${String(depTm?.getHours()).padStart(2, '0')}:${String(depTm?.getMinutes()).padStart(2, '0')}:00.000Z`;
        const tmpToDate = `${arrDate?.getFullYear()}-${String(parseInt(arrDate?.getMonth(), 10) + 1).padStart(2, '0')}-${String(arrDate?.getDate()).padStart(2, '0')}T${String(arrTm?.getHours()).padStart(2, '0')}:${String(arrTm?.getMinutes()).padStart(2, '0')}:00.000Z`;
    ...
    const handleUseRec = () => {
        ...
        if (jsonData.drv_pid === 0 || jsonData.car_type_id === 0 ||
            jsonData.car_id === 0) {
            setSubmitINC(true);
            return;
        }
                ...
                <DialogContent>
                    <Grid container rowSpacing={{ xs: 2 }} columnSpacing={{ xs: 2 }}>
                        <Grid item md={12}>
                            ...
                            {duplicate ?
                                <Grid item xs={12}>
                                    <Typography variant='subtitle1' color={'error.main'} > รถคันนี้มีการใช้งานซ้ำกับคำขอเลขที่ {duplicate}<br />จาก {dupDept} </Typography>
                                </Grid>
                                :
                                ""}
                        </Grid>
