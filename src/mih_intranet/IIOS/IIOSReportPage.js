...
// @mui
import { Stack, Typography, Divider, TextField, Button, alpha, styled, Box, Checkbox, Grid } from '@mui/material';
...
function IIOSReport() {
  ...
  const handleFindDate = () => {
    ...
    if (isIncomplete) {
      setFilterTaskList(
        taskList.filter(
          (dt) =>
            dt.task_date_start >= tmpFromDate &&
            dt.task_date_start <= dateFns.format(dateFns.addDays(new Date(toDate), 1), 'yyyy-MM-dd')
        )
      );
    } else {
      setFilterTaskList(
        completeTaskList.filter(
          (dt) =>
            dt.task_date_start >= tmpFromDate &&
            dt.task_date_start <= dateFns.format(dateFns.addDays(new Date(toDate), 1), 'yyyy-MM-dd')
        )
  ...
  const handleOnlyIncomplete = (event) => {
    ...
    if (event.target.checked) {
      ...
      if (mode === 'byId') {
        setFilterTaskList(
          completeTaskList.filter((dt) => dt.task_id.includes(document.getElementById('taskId').value))
        );
      } else if (mode === 'byDate') {
        setFilterTaskList(
          completeTaskList.filter(
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: 'ผู้แจ้ง',
      width: 100,
      valueGetter: (params) => `${params.row.informer_firstname} ${params.row.informer_lastname}`,
    },
    {
    ...
    },
    {
      ...
      width: 120,
      valueGetter: (params) => `${params.row.task_date_accept.replace('T', ' ').replace('.000Z', ' น.')}`,
    },
    {
      field: 'task_date_process',
      headerName: 'วันที่ดำเนินการล่าสุด',
      width: 120,
      valueGetter: (params) => `${params.row.task_date_process.replace('T', ' ').replace('.000Z', ' น.')}`,
    },
    {
      field: 'task_date_end',
        ...
        `${
          params.row.task_iscomplete === null || params.row.task_iscomplete === ''
            ? params.row.status_id_request === null || params.row.status_id_request === ''
              ...
              : `${params.row.status_name} (รออนุมัติ)`
    ...
    },
    {
      ...
      valueGetter: (params) =>
        `${
          params.row.receiver_firstname === null || params.row.receiver_firstname === ''
            ? ''
            : `${params.row.receiver_firstname} ${params.row.receiver_lastname}`
        }`,
    },
    {
      ...
      valueGetter: (params) =>
        `${
          params.row.operator_firstname === null || params.row.operator_firstname === ''
            ? ''
            : `${params.row.operator_firstname} ${params.row.operator_lastname}`
        }`,
    },
    {
                  ...
                  <Stack direction="row" spacing={10} sx={{ width: 'auto', p: 2 }}>
                    <Box>
                      ...
                      <Stack direction="row" spacing={1}>
                        ...
                        <Button variant="contained" sx={{ width: 100 }} onClick={handleFindTaskId}>
                          ค้นหา
                        </Button>
                      </Stack>
                    </Box>
                  ...
                  <Stack spacing={2} sx={{ width: 'auto', p: 2 }}>
                    <Grid container>
                      ...
                      <Grid item sm={12} md={2.5}>
                        <Box>
                          <Checkbox onChange={handleOnlyIncomplete} sx={{ '& .MuiSvgIcon-root': { fontSize: 28 } }} />
                          ซ่อนงานที่ดำเนินการเสร็จสิ้น
                        </Box>
                      </Grid>
                    </Grid>
                    <div style={{ width: '100%' }}>
