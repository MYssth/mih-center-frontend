...
function IIOSTaskAcpt({ openDialg, onCloseDialg, data, operList, estList, recvId }) {
  ...
  const handleAcceptTask = () => {
    const jsonData = {
      ...
      receiver_id: recvId,
      receiver_name: `${operList.find((o) => o.personnel_id === recvId).personnel_firstname} ${
        operList.find((o) => o.personnel_id === recvId).personnel_lastname
      }`,
    ...
    fetch(`${process.env.REACT_APP_host}${process.env.REACT_APP_dmisPort}/accepttask`, {
      method: 'POST',
      headers: {
    ...
    <>
      ...
      >
        ...
        <DialogContent>
          ...
          />
          <DialogContentText>กรุณาระบุการประมาณวันดำเนินงาน</DialogContentText>
          <Autocomplete
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleAcceptTask}>
            รับเรื่อง
          </Button>
        </DialogActions>
      </Dialog>
